...
class BadmintonController extends Controller
{
    ...
    public function addSpace(Request $request)
    {
        ...
        return redirect('/space/badminton/overview')->with('success', '球場新增成功！');
    ...
    public function editSpaceShow($spaceId)
    {
        ...
        return view('badminton/edit', compact('court')); // Pass the court data to the view
    }
    

    public function editSpace(Request $request, $spaceId)
    {
        Log::info('editSpace 羽球執行');
        Log::info('接收到的數據', $request->all());
        ...
        return redirect('/space/badminton/overview')->with('success', '球場更新成功！');
    }
    ...
    public function deleteSpace($spaceId)
    {
        try {
            ...
            return redirect('/space/badminton/overview')->with('success', '球場刪除成功！');
        } catch (\Exception $e) {
    ...
    public function overviewSpace(){
        ...
        // 取得所有類型為羽球的球場 type = 4
        $basketballCourts = Court::with('ballType')->where('type', 4)->get();
        ...
        return view('badminton/overview', compact('courts'));
